...
class Download:
    ...
    def download(self, params, filefunc, statusfunc, finfunc, errorfunc, doneflag, cols,
        ...
        self.ratelimiter = RateLimiter(rawserver.add_task,
                                       config['upload_unit_size'])
        self.ratelimiter.set_upload_rate(config['max_upload_rate'])
        
        def make_upload(connection, ratelimiter, totalup, choker = choker,
                ...
                buffer_reads = config['buffer_reads']):
            return Upload(connection, ratelimiter, totalup,
                          choker, storagewrapper, picker,
        ...
        connecter = Connecter(make_upload, downloader, choker,
            len(pieces), upmeasure, config, self.ratelimiter, rawserver.add_task)
        ...
        rerequest = Rerequester(trackerlist, config['rerequest_interval'], 
            ...
            config['min_peers'], encoder.start_connections, 
            rawserver.external_add_task, storagewrapper.get_amount_left, 
            upmeasure.get_total, downmeasure.get_total, listen_port, 
            config['ip'], myid, infohash, config['http_timeout'],
            errorfunc, exchandler, config['max_initiate'], doneflag,
        ...
        def rerequest_lastfailed(rerequest = rerequest):
            return rerequest.last_failed
        statistics = Statistics(upmeasure,downmeasure,connecter,
                            httpdownloader,rerequest_lastfailed,self.filedatflag)
        if info.has_key('files'):
            statistics.set_dirstats(files, len(pieces), info['piece length'])

        DownloaderFeedback(choker, httpdownloader, rawserver.add_task,
            upmeasure.get_rate, downmeasure.get_rate, ratemeasure.get_time_left, 
            ratemeasure.get_size_left, file_length, finflag, self.spewflag,
            statistics, statusfunc, config['display_interval'])
